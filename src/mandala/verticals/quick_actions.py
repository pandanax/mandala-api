...
_ASTROLOGY: dict[str, str] = {
    ...
    "mdl:matrix": (
        "Растолкуй мою Карту судьбы (Матрицу Судьбы) по рассчитанным арканам: "
        "личностный квадрат (портрет, таланты, кармическая задача, зона комфорта), "
        "предназначение, денежный канал и канал отношений. Значения арканов возьми из "
        "базы знаний, числа не пересчитывай."
    ),
    "mdl:numerology": (
        "Растолкуй мою пифагорейскую нумерологию по рассчитанным числам: жизненный путь, "
        "число выражения (судьбы), число души, число личности, число дня рождения и "
        "число зрелости; отдельно поясни мастер-числа 11/22/33, если есть. Значения "
        "чисел возьми из базы знаний, числа не пересчитывай."
    ),
    ...
    # Контекстный фолбэк навигации, когда модель не дала свой nav-блок. Не одна кнопка,
    # а осмысленный минимум переходов — продолжить тему / другой аспект / к темам.
    "mdl:continue": (
        "Продолжи и углуби текущую тему разбора моей натальной карты — расскажи подробнее."
    ),
    "mdl:another": (
        "Разбери другой аспект моей натальной карты — предложи, что ещё интересного "
        "можно посмотреть, и разбери одно из этого."
    ),
    # Возврат «к темам» → детерминированное БОГАТОЕ меню тем инлайн-кнопками (не текст
    # к LLM): гарантируем богатый рендер каждый раз, не полагаясь на nav-блок модели.
    # Разворачивается в спец-код и перехватывается в domain/handler ДО вызова LLM.
    "mdl:topics": "__topics_menu__",
    # Профиль и сброс
...
# Код-триггер детерминированного меню тем «⬅️ К темам» (не передаётся в LLM)
TOPICS_MENU_CODE = "__topics_menu__"
...
def is_topics_menu(text: str | None) -> bool:
    """Вернуть True если нажата кнопка «⬅️ К темам» (показать меню тем разбора)."""
    return text is not None and text.strip() == TOPICS_MENU_CODE
