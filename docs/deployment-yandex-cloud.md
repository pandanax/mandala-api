...
### 5.1. Контракт PORT / EXPOSE / healthcheck

Один образ (`Containerfile`) обслуживает два контура запуска; приложение биндит `${PORT}`
(`src/mandala/http/__main__.py`).

| Контур | Как задаётся порт | Значение |
|--------|-------------------|----------|
| **ВМ + nginx** | `scripts/deploy/restart_app.sh` запускает с `-e PORT=8000 -p 8000:8000` | 8000 |
| **YC Serverless Container** | берёт дефолт образа; YC маршрутизирует на порт `EXPOSE` | 8080 |

Правила контракта:

- **`EXPOSE` обязан совпадать с дефолтным `PORT`** (оба **8080**) — YC роутит именно на
  `EXPOSE`.
- **`HEALTHCHECK` обязан пробить `${PORT}`, а не хардкод-порт.** Иначе на ВМ (`PORT=8000`)
  probe стучится в 8080 и контейнер репортит `unhealthy`. Проверено: старый хардкод-8080
  → `unhealthy` на ВМ; probe по `${PORT}` → `healthy` на обоих контурах.
- Здоровье приложения — эндпоинт **`/health`** (`{"status":"ok","database":"ok"}`), его же
  ждут `deploy.sh` / `restart_app.sh` после пересоздания контейнера.
