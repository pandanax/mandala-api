...
- **Статические точки входа живут в бургер-меню** (`setMyCommands` в `bot_commands.py`):
  «Натальная карта» (`/natal`), «Прогноз» (`/forecast`), профиль/рестарт/help/промо/тарифы.
...
- **Крайний фолбэк astrology — одна кнопка «⬅️ К темам»** (`mdl:topics`), а не прежние
  статические кнопки-действия. Клик просит модель предложить темы разбора — и она снова
  выдаёт свежую контекстную навигацию. Так ответ без навигации не остаётся, но и не несёт
  чужих пресных кнопок.
